...
export const useComments = (initialComments: ProjectComments = {}) => {
  ...
  const isPendingComment = useCallback((id: string | number) => {
  ...
  const handleNewComment = useCallback((comment: CommentType) => {
    ...
    if (isPendingComment(comment.id)) {
      removePendingCommentId(comment.id);
      return;
    }
  ...
  const createCommentMutation = useMutation({
    ...
    onMutate: async ({ projectId, content }, { user }) => {
      if (!user) return null;
